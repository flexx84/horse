...
def get_all_links(html_content, base_url):
    ...
    links = set()
    url_attrs = ["href", "src", "action", "data-href", "data-src", "data-url", "data-link", "data"]
    for tag in soup.find_all(True):
        for attr in url_attrs:
            if tag.has_attr(attr):
                url_candidate = tag.get(attr)
                if url_candidate:
                    # Skip JavaScript code and invalid patterns
                    if any(js_pattern in url_candidate.lower() for js_pattern in [
                        "javascript:", "location.href", "popwindowclose", "alert(", 
                        "modal_alert", "foutlogin_submit", "window.open", "("
                    ]):
                        continue
                    
                    # Skip URLs that are too long (likely corrupted)
                    if len(url_candidate) > 200:
                        continue
                    
                    # Skip URLs with too many slashes (likely corrupted)
                    if url_candidate.count("/") > 10:
                        continue
