...
async function executeNaverSearch(slot, options = {}) {
  ...
  try {
    ...
    try {
      ...
      const autocompleteItems = await page.$$eval('.autocomplete .item', items =>
        items.map((item, idx) => ({
          index: idx,
      ...
      if (targetItem) {
        ...
        if (remainingText) {
          await page.type('#query', ' ' + remainingText, { delay: 100 });
        }
    ...
    if (options.targetDomain) {
      ...
      const results = await page.$$eval('.lst_total a', links =>
        links.map(a => ({
...
async function main() {
  console.log('\n' + '='.repeat(60));
  console.log('🤖 Impact 네이버 검색 봇 시작');
  console.log('='.repeat(60) + '\n');
